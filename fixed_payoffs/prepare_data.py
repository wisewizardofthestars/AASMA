...
def load_fixed_payoff_data(csv_dir=None):
    """
    Loads and merges all fixed-payoff tournament CSVs into a single DataFrame.
    Adds a 'tournament_type' column for downstream analysis.
    """
    if csv_dir is None:
...
if __name__ == "__main__":
    ...
    df = load_fixed_payoff_data()
    print(df.head())
    # Optionally save merged data for fast access
    df.to_csv(os.path.join(os.path.dirname(__file__), 'fixed_payoff_merged.csv'), index=False)